 ...
 * Retries if the backend check hasn't completed yet.
...
(function() {
    ...
    function doCheck(attempt) {
        fetch('/api/update-check')
            .then(function(r) { return r.json(); })
            .then(function(data) {
                if (data.pending && attempt < MAX_RETRIES) {
                    setTimeout(function() { doCheck(attempt + 1); }, RETRY_DELAY);
                    return;
                }
